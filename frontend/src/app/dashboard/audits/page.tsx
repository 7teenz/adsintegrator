...
export default function AuditsPage() {
  ...
  if (loading) {
    return <div className="flex justify-center py-20"><Spinner label="Loading audits..." /></div>;
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              {report ? verdict : "Run an audit after upload to turn your Meta export into a focused executive report."}
            </p>
          </div>
          <button
            type="button"
          ...
          <section className="grid gap-4 xl:grid-cols-12">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="mt-2 text-xs font-semibold text-slate-500">
                            Why this matters: {action.impactValue > 0 ? `${formatCurrency(action.impactValue)} of modeled impact is tied to this issue.` : "It is one of the clearest performance drags in the current audit."}
                          </p>
                        </div>
                      </div>
                    </article>
            ...
            <div className="xl:col-span-5 space-y-4">
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {biggestLeak ? (
                  <>
                    <h3 className="mt-1 text-lg font-semibold text-slate-900">{biggestLeak.title}</h3>
                    ...
                    <div className="mt-4 flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                      ...
                      <div className="text-right">
                        <p className="text-xs uppercase tracking-[0.14em] text-slate-500">Estimated waste</p>
                        <p className="mt-1 text-sm font-semibold text-rose-700">{formatCurrency(biggestLeak.waste)}</p>
                      </div>
                    </div>
                  </>
                ) : (
